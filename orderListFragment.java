...
public class orderListFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root=inflater.inflate(R.layout.order_list, container, false);
        try
        {
            ...
            public void onClick(View v) {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if(Ord_action.equalsIgnoreCase("Order To Kitchen"))
                        builder.setMessage("Are You Sure Order To Kitchen?");
                        else if(Ord_action.equalsIgnoreCase("Cancel"))
                            builder.setMessage("Are You Sure? Do You Want To Cancel This Order");
                            ...
                            public void onClick(DialogInterface dialog, int id)
                            {
                                try
                                {
                                    if(MainActivity.Connection_type.equalsIgnoreCase("LAN(PC)"))
                                    {
                                        ...
                                        if (Ord_action.equalsIgnoreCase("Order To Kitchen")) {
                                            st.execute("update voucher1 set order_status='On Process' where orderTake='" + MainActivity.Emp_USERNAME + "' and voucherId='" + Order_id + "'");

                                            appendRow(table);
                                        } else if (Ord_action.equalsIgnoreCase("Cancel")) {
                                            st.execute("update voucher1 set order_status='Order Cancel' where orderTake='" + MainActivity.Emp_USERNAME + "' and voucherId='" + Order_id + "'");
                                            appendRow(table);
                                        }
                                    }
                                    else if(MainActivity.Connection_type.equalsIgnoreCase("LAN(Mobile)"))
                                    {
                                        ...
                                        if (Ord_action.equalsIgnoreCase("Order To Kitchen")) {
                                            //dbManager.update4(orderID, "On Process");
                                            appendRow(table);
                                        } else if (Ord_action.equalsIgnoreCase("Cancel")) {
                                            //dbManager.update4(orderID, "Order Cancel");
                                            appendRow(table);
                                        }
                                    }
                                }
        ...
        try
        {
            ...
            if(MainActivity.Connection_type.equalsIgnoreCase("LAN(PC)"))
            {
                ...
                if(MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Cashier")||MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Manager")||MainActivity.Emp_USERNAME.equalsIgnoreCase("admin"))
                    rs=st.executeQuery("select * from voucher1 where status='true' and (order_status='Ordered' or order_status='Ready For Kitchen') order by timeStamp desc");
                else
                    rs=st.executeQuery("select * from voucher1 where orderTake='"+MainActivity.Emp_USERNAME+"' and status='true' and (order_status='Ordered' or order_status='Ready For Kitchen') order by timeStamp desc");
                while (rs.next())
                {
                    s = new ArrayList();
            ...
            else if(MainActivity.Connection_type.equalsIgnoreCase("LAN(Mobile)"))
            {
                ...
                if(MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Manager"))
                    cr = DBManager.database.rawQuery("SELECT " + DatabaseHelper.d0+","+DatabaseHelper.d20+","+DatabaseHelper.d7+","+DatabaseHelper.d9+","+DatabaseHelper.d16+" FROM " + DatabaseHelper.TABLE_NAME3+" where "+DatabaseHelper.d21+"='true' and ("+DatabaseHelper.d24+"='Ordered' or "+DatabaseHelper.d24+"='Ready For Kitchen') order by "+DatabaseHelper.d23+" desc,"+DatabaseHelper.d16+" desc", null);
                else
                    cr = DBManager.database.rawQuery("SELECT " + DatabaseHelper.d0+","+DatabaseHelper.d20+","+DatabaseHelper.d7+","+DatabaseHelper.d9+","+DatabaseHelper.d16+" FROM " + DatabaseHelper.TABLE_NAME3+" where "+DatabaseHelper.d8+"='"+ MainActivity.Emp_USERNAME +"' and "+DatabaseHelper.d21+"='true' and ("+DatabaseHelper.d24+"='Ordered' or "+DatabaseHelper.d24+"='Ready For Kitchen') order by "+DatabaseHelper.d23+" desc,"+DatabaseHelper.d16+" desc", null);
                while(cr.moveToNext())
                {
                    s = new ArrayList();
    ...
    private void appendRow(TableLayout table) {
        try
        {
            ...
            for(int i=0;i<b.size();i++)
            {
                ...
                l1 = new TextView(root.getContext());
                l1.setTextColor(Color.BLACK);
                l1.setBackgroundResource(R.drawable.round_corner2);
                l1.setText("Cancel");
                l1.setTextSize(14);
                l1.setTypeface(null, Typeface.BOLD);
                l1.setTooltipText(tolTx);
