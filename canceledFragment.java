...
public class canceledFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root=inflater.inflate(R.layout.canceled_process, container, false);
        try
        {
        ...
        try
        {
            ...
            if(MainActivity.Connection_type.equalsIgnoreCase("LAN(PC)"))
            {
                ...
                if(MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Cashier")||MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Manager")||MainActivity.Emp_USERNAME.equalsIgnoreCase("admin"))
                    rs=st.executeQuery("select * from voucher1 where status='true' and (order_status='Process Cancel' or order_status='Order Cancel') order by timeStamp desc");
                else
                    rs=st.executeQuery("select * from voucher1 where orderTake='"+MainActivity.Emp_USERNAME+"' and status='true' and (order_status='Process Cancel' or order_status='Order Cancel') order by timeStamp desc");
                while (rs.next())
                {
                    s = new ArrayList();
                    ...
                    s.add(rs.getString("order_status"));
            ...
            {
                /*
                Cursor cr = DBManager.database.rawQuery("SELECT " + DatabaseHelper.d0+","+DatabaseHelper.d20+","+DatabaseHelper.d7+","+DatabaseHelper.d9+","+DatabaseHelper.d16+","+DatabaseHelper.d24+" FROM " + DatabaseHelper.TABLE_NAME3+" where "+DatabaseHelper.d8+"='"+ MainActivity.Emp_USERNAME +"' and "+DatabaseHelper.d21+"='true' and ("+DatabaseHelper.d24+"='Process Cancel' or "+DatabaseHelper.d24+"='Order Cancel') order by "+DatabaseHelper.d23+" desc,"+DatabaseHelper.d16+" desc", null);
                while(cr.moveToNext())
                {
                    s = new ArrayList();
    ...
    private void appendRow(TableLayout table) {
        try
        {
            ...
            for(int i=0;i<b.size();i++)
            {
                ...
                String orderS=s.get(5).toString();
                ...
                ArrayList s1= (ArrayList) s.get(6);
                ...
                l1.setGravity(Gravity.CENTER|Gravity.CENTER_HORIZONTAL);
                l1.setOnClickListener(txClick);
                butLin.addView(l1,qtysubParms1);
