...
public class onProcessFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root=inflater.inflate(R.layout.onprocess_order, container, false);
        try
        {
        ...
        txClick=new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try
                {
                    ...
                    Toast.makeText(root.getContext(),"ID="+Order_id+" act="+Ord_action,Toast.LENGTH_LONG).show();
                    ...
                    else
                    {
                        ...
                        if(Ord_action.equalsIgnoreCase("Order Done"))
                            ...
                            public void onClick(DialogInterface dialog, int id)
                            {
                                try
                                {
                                    if(MainActivity.Connection_type.equalsIgnoreCase("LAN(PC)"))
                                    {
                                        ...
                                        if (Ord_action.equalsIgnoreCase("Order Done")) {
                                            st.execute("update voucher1 set order_status='Done' where orderTake='" + MainActivity.Emp_USERNAME + "' and voucherId='" + Order_id + "'");

                                            appendRow(table);
                                        } else if (Ord_action.equalsIgnoreCase("Cancel")) {
                                            st.execute("update voucher1 set order_status='Process Cancel' where orderTake='" + MainActivity.Emp_USERNAME + "' and voucherId='" + Order_id + "'");
                                            appendRow(table);
                                        }
                                    }
                                    else if(MainActivity.Connection_type.equalsIgnoreCase("LAN(Mobile)"))
                                    {
                                        ...
                                        if(Ord_action.equalsIgnoreCase("Order Done"))
                                        {
                                            //dbManager.update4(orderID,"Done");
                                            appendRow(table);
                                        }
                                        else if(Ord_action.equalsIgnoreCase("Cancel"))
                                        {
                                            //dbManager.update4(orderID,"Process Cancel");
                                            appendRow(table);
                                        }
                                    }
                                }
        ...
        try
        {
            ...
            if(MainActivity.Connection_type.equalsIgnoreCase("LAN(PC)"))
            {
                ...
                if(MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Cashier")||MainActivity.Emp_PRIVILAGE.equalsIgnoreCase("Manager")||MainActivity.Emp_USERNAME.equalsIgnoreCase("admin"))
                    rs=st.executeQuery("select * from voucher1 where status='true' and order_status='On Process' order by timeStamp desc");
                else
                    rs=st.executeQuery("select * from voucher1 where orderTake='"+MainActivity.Emp_USERNAME+"' and status='true' and order_status='On Process' order by timeStamp desc");
                while (rs.next())
                {
                    s = new ArrayList();
            ...
            {/*
                Cursor cr = DBManager.database.rawQuery("SELECT " + DatabaseHelper.d0+","+DatabaseHelper.d20+","+DatabaseHelper.d7+","+DatabaseHelper.d9+","+DatabaseHelper.d16+" FROM " + DatabaseHelper.TABLE_NAME3+" where "+DatabaseHelper.d8+"='"+ MainActivity.Emp_USERNAME +"' and "+DatabaseHelper.d21+"='true' and "+DatabaseHelper.d24+"='On Process' order by "+DatabaseHelper.d23+" desc,"+DatabaseHelper.d16+" desc", null);
                while(cr.moveToNext())
                {
                    s = new ArrayList();
